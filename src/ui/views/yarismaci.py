...
def render_vitrin(st_ctx, current_user: dict, lang: str = "tr") -> None:
    ...
    _btn_card_lbl = "› Specs & Details" if lang == "en" else "› Şartname & Detaylar"
    _ribbon_lbl   = "New"              if lang == "en" else "Yeni"
    ...
    for i in range(0, len(gosterilecek), 3):
        ...
        for idx, comp in enumerate(row):
            with cols[idx]:
                ...
                name_tr = _attr(comp, "name", "")
                name_en_val = _attr(comp, "name_en", "") or ""
                name = (name_en_val if name_en_val else name_tr) if lang == "en" else name_tr
                is_featured = (idx % 2 == 0)
                ...
                ribbon_html = f'<div class="ts-corner-ribbon">★<br/>{_ribbon_lbl}</div>' if is_featured else ''
