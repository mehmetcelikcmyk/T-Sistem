...
def _render_create_form(st_ctx, actor: str, lang: str) -> None:
    ...
    with st_ctx.form("adm_form_new_comp"):
        ...
        name_en = col2.text_input("İngilizce Ad (English Name)", placeholder="e.g. Autonomous Vehicle Competition")
        col1b, col2b = st_ctx.columns(2)
        domain = col1b.selectbox(t("lbl_domain_new", lang), DOMAIN_SUGGESTIONS)
        ...
        if st_ctx.form_submit_button(t("btn_create", lang), type="primary"):
            ...
            comp = Competition(
                competition_id=slug, name=name.strip(), name_en=name_en.strip() or None,
...
def _render_general(st_ctx, actor: str, lang: str, comp: Competition) -> None:
    ...
    with st_ctx.form(f"adm_general_{comp.competition_id}"):
        ...
        name_en = col2.text_input("İngilizce Ad (English Name)", value=getattr(comp, "name_en", None) or "")
        domain_options = DOMAIN_SUGGESTIONS + ([comp.domain] if comp.domain not in DOMAIN_SUGGESTIONS else [])
        domain = col1b.selectbox(t("lbl_domain", lang), domain_options,
