...
def render_landing_view() -> None:
    ...
    * {
    ...
    /* TR / EN Dil Değiştirici — Uzay / Neon Teması */
    div.st-key-landing_lang_btn > button {
        background: rgba(2, 28, 97, 0.55) !important;
        border: 1.5px solid #3DD3FF !important;
        color: #3DD3FF !important;
        font-weight: 900 !important;
        ...
        box-shadow: 0 0 12px rgba(61, 211, 255, 0.30), inset 0 0 8px rgba(61, 211, 255, 0.08) !important;
        transition: all 0.20s ease !important;
        min-height: 38px !important;
        text-transform: uppercase !important;
    }
    div.st-key-landing_lang_btn > button:hover {
        background: rgba(61, 211, 255, 0.18) !important;
        border-color: #FFFFFF !important;
        color: #FFFFFF !important;
        box-shadow: 0 0 22px rgba(61, 211, 255, 0.60) !important;
    ...
    # 1. ÜST HEADER (NAVBAR)
    # Dil durumu
    _lang = st.session_state.get("lang", "tr")

    _lbl_ann    = "DUYURULAR"        if _lang == "tr" else "ANNOUNCEMENTS"
    _lbl_panel  = "PANELİME GİT →"  if _lang == "tr" else "MY PANEL →"
    _lbl_login  = "GİRİŞ YAP"       if _lang == "tr" else "SIGN IN"
    _lbl_sub    = ("Yapay Zekâ Destekli 4. Göz Rapor Değerlendirme İstasyonu"
                   if _lang == "tr"
                   else "AI-Powered 4th Eye Report Evaluation Station")
    _lbl_brand  = "T-SİSTEM"        # İsim hiçbir zaman değişmez
    _next_lang  = "en"              if _lang == "tr" else "tr"
    _lbl_switch = "🌐 EN"            if _lang == "tr" else "🌐 TR"

    # Slider & DETAYLAR
    _lbl_explore = "↓ Yarışmayı İncele"  if _lang == "tr" else "↓ Explore Competition"
    _lbl_details = "DETAYLAR"            if _lang == "tr" else "DETAILS"

    # İstatistik kartları
    _lbl_stat1_lbl = "Yarışma Kapsamı"        if _lang == "tr" else "Competition Scope"
    _lbl_stat1_sub = "Tüm TEKNOFEST Alanları" if _lang == "tr" else "All TEKNOFEST Fields"
    _lbl_stat2_lbl = "AI 4. Göz Doğruluğu"   if _lang == "tr" else "AI 4th Eye Accuracy"
    _lbl_stat2_sub = "Şartname Kural Denetimi" if _lang == "tr" else "Rule Compliance Check"
    _lbl_stat3_lbl = "İntihal Analizi"         if _lang == "tr" else "Plagiarism Analysis"
    _lbl_stat3_sub = "Vektörel Benzerlik Matrisi" if _lang == "tr" else "Vector Similarity Matrix"
    _lbl_stat4_lbl = "Karar Güvenliği"         if _lang == "tr" else "Decision Security"
    _lbl_stat4_val = "Mühürlü"                 if _lang == "tr" else "Sealed"
    _lbl_stat4_sub = "Resmî Hakem Karnesi"     if _lang == "tr" else "Official Referee Scorecard"

    # Duyurular bölümü
    _lbl_ann_section = ("Resmî Duyurular ve Yarışma Bildirimleri"
                        if _lang == "tr" else
                        "Official Announcements & Competition Notifications")
    _lbl_all_ann  = "Tüm Duyurular →"      if _lang == "tr" else "All Announcements →"
    _lbl_featured = "ÖNE ÇIKAN"            if _lang == "tr" else "FEATURED"
    _lbl_read_ann = "Duyuruyu Oku ➔"       if _lang == "tr" else "Read More ➔"
    _lbl_no_ann   = ("Şu anda yayında aktif bir duyuru bulunmuyor."
                     if _lang == "tr" else
                     "No active announcements at this time.")

    # Geri sayım
    _cd_region   = "GÜNEYDOĞU"             if _lang == "tr" else "SOUTHEAST"
    _cd_loc      = "Şanlıurfa GAP Havalimanı" if _lang == "tr" else "Şanlıurfa GAP Airport"
    _cd_days_word = "Gün"                  if _lang == "tr" else "Days"

    # Alt çağrı (CTA)
    _lbl_cta_title = ("T-Sistem Değerlendirme Masasına Katılın"
                      if _lang == "tr" else
                      "Join the T-Sistem Evaluation Platform")
    _lbl_cta_desc  = ("Yarışmacı takımlar aşama raporlarını yükleyebilir, hakemler 4. göz yapay zekâ "
                       "asistanıyla puanlama yapabilir ve yöneticiler tüm süreci yönetebilir."
                      if _lang == "tr" else
                      "Competing teams can upload phase reports, referees can score with the AI 4th eye "
                      "assistant, and administrators can manage the entire process.")
    _lbl_bottom_panel = "Panelime Dön →"   if _lang == "tr" else "Go to My Panel →"
    _lbl_bottom_login = "Giriş Yaparak Başla" if _lang == "tr" else "Sign In to Get Started"

    # Dil değiştirici — navbar ÜSTÜNDE, sağ köşede (authenticated sayfasıyla aynı konum)
    _ls, _lc = st.columns([5.5, 0.5])
    with _lc:
        if st.button(_lbl_switch, key="landing_lang_btn", use_container_width=True):
            st.session_state.lang = _next_lang
            for _fk in ["yarismaci_alan_filtre", "yarismaci_seviye_filtre"]:
    ...
    with st.container():
        ...
        with nav_c1:
            ...
            header_brand_html = (
                ...
                f'{logo_badge_html}'
                '<div>'
                f'<div style="font-size:2.25rem; font-weight:900; color:#FFFFFF; letter-spacing:-0.02em; line-height:1.1; text-shadow:0 2px 12px rgba(0,0,0,0.6);">'
                f'<span style="color:#FFA500;">{_lbl_brand}</span>'
                '</div>'
                f'<div style="font-size:0.84rem; font-weight:800; color:#3DD3FF; letter-spacing:0.08em; text-transform:uppercase; margin-top:4px;">'
                f'{_lbl_sub}'
                '</div>'
                '</div>'
                '</div>'
        ...
        with nav_c2:
            ...
            with b_c1:
                if st.button(_lbl_ann, key="tf_nav_announcements_top", type="secondary", use_container_width=True):
                    st.query_params["view"] = "ann"
                    st.rerun()
            with b_c2:
                is_auth = bool(st.session_state.get("authenticated", False))
                if is_auth:
                    if st.button(_lbl_panel, key="tf_nav_panel_top", type="primary", use_container_width=True):
                        st.query_params.clear()
                        st.query_params["tab"] = "ana_sayfa"
                        st.rerun()
                else:
                    if st.button(_lbl_login, key="tf_nav_login_top", type="primary", use_container_width=True):
                        st.query_params["view"] = "login"
                        st.rerun()

    # GENİŞ BOŞLUK: Navbar -> Slider
    ...
    div[data-testid="stButton"] > button[kind="primary"] {
        ...
        box-shadow: 0 4px 18px rgba(222, 56, 15, 0.6) !important;
    ...
    /* Sadece carousel ok tuşlarına uygula — diğer secondary butonlara dokunma */
    div.st-key-btn_prev_comp button, div.st-key-btn_next_comp button {
    ...
    /* Yarışma Geçiş Animasyonları */
    @keyframes floatCircle {
        0%, 100% { transform: translateY(0px); box-shadow: 0 0 40px rgba(61, 211, 255, 0.30); }
        50%       { transform: translateY(-14px); box-shadow: 0 0 65px rgba(61, 211, 255, 0.58); }
    }
    ...
    .tf-circle-aura {
        animation: floatCircle 3.6s ease-in-out infinite;
    }
    .tf-comp-title {
        animation: fadeSlideIn 0.42s ease-out;
    }
    .tf-comp-desc {
        animation: fadeSlideIn 0.52s ease-out;
    }
    </style>
    """, unsafe_allow_html=True)
    ...
    with c_prev:
        if st.button("❮", key="btn_prev_comp", type="secondary", help="Önceki Yarışma"):
            st.session_state.landing_slider_idx -= 1
            st.rerun()

    with c_content:
        c_left, c_right = st.columns([1.3, 0.9], vertical_alignment="center")
        slug_to_open = c_slug or "havacilikta-yapay-zeka"
        with c_left:
            st.markdown(f'<div class="tf-comp-title">{c_title}</div><div class="tf-comp-desc">{c_desc}</div>', unsafe_allow_html=True)

        with c_right:
            st.markdown(f"""
            <div class="tf-circle-aura">
                <img class="tf-circle-logo" src="{c_logo}" alt="Yarışma Logo"/>
            </div>
            """, unsafe_allow_html=True)

    with c_next:
        if st.button("❯", key="btn_next_comp", type="secondary", help="Sonraki Yarışma"):
            st.session_state.landing_slider_idx += 1
            st.rerun()

    nav_slug = slug_to_open or "havacilikta-yapay-zeka"

    # DETAYLAR — slider DIŞINDA, ortalanmış, yüzen animasyonlu bölüm
    st.markdown(f"""
    <style>
    @keyframes floatBtn {{
        0%, 100% {{ transform: translateY(0px); box-shadow: 0 6px 28px rgba(222,56,15,0.55); }}
        50%  {{ transform: translateY(-10px); box-shadow: 0 16px 40px rgba(222,56,15,0.80); }}
    }}
    .tf-detaylar-wrap {{
        display: flex;
        flex-direction: column;
    ...
    .tf-detaylar-link {{
        ...
        padding: 15px 56px;
        border-radius: 10px;
        border: 1.5px solid rgba(255,120,80,0.5);
        animation: floatBtn 2.8s ease-in-out infinite;
        cursor: pointer;
    }}
    .tf-detaylar-link:hover {{
        background: linear-gradient(135deg, #FF3300 0%, #FF0000 100%);
        color: #FFFFFF !important;
    }}
    </style>
    <div class="tf-detaylar-wrap">
        <span class="tf-detaylar-label">{_lbl_explore}</span>
        <a class="tf-detaylar-link" href="/?view=comp&slug={nav_slug}" target="_top">{_lbl_details}</a>
    </div>
    """, unsafe_allow_html=True)
    ...
    * {
        ...
        <div class="tf-radar-box" id="radarContainer">
            ...
            <div class="tf-radar-content">
                <div>
                    ...
                    <div class="tf-left-sub">{_cd_region}</div>
                    <div class="tf-left-loc">{_cd_loc}</div>
                </div>
                <div>
    ...
    """
    countdown_component_code = (countdown_component_code
        .replace("{_cd_region}", _cd_region)
        .replace("{_cd_loc}", _cd_loc)
        .replace("' Gün'", f"' {_cd_days_word}'")
        .replace("29 Gün", f"29 {_cd_days_word}")
    )
    ...
    with m1:
        ...
        <div class="tf-mini-stat">
            <div class="tf-mini-stat-lbl">{_lbl_stat1_lbl}</div>
            <div class="tf-mini-stat-num">{total_comps}</div>
            <div style="font-size:0.72rem; color:#3DD3FF; font-weight:700;">{_lbl_stat1_sub}</div>
        </div>
        """, unsafe_allow_html=True)
    with m2:
        st.markdown(f"""
        <div class="tf-mini-stat">
            <div class="tf-mini-stat-lbl">{_lbl_stat2_lbl}</div>
            <div class="tf-mini-stat-num">%98.4</div>
            <div style="font-size:0.72rem; color:#CBD5E1; font-weight:600;">{_lbl_stat2_sub}</div>
        </div>
        """, unsafe_allow_html=True)
    with m3:
        st.markdown(f"""
        <div class="tf-mini-stat">
            <div class="tf-mini-stat-lbl">{_lbl_stat3_lbl}</div>
            <div class="tf-mini-stat-num">Semantic</div>
            <div style="font-size:0.72rem; color:#FFA07A; font-weight:600;">{_lbl_stat3_sub}</div>
        </div>
        """, unsafe_allow_html=True)
    with m4:
        st.markdown(f"""
        <div class="tf-mini-stat">
            <div class="tf-mini-stat-lbl">{_lbl_stat4_lbl}</div>
            <div class="tf-mini-stat-num">{_lbl_stat4_val}</div>
            <div style="font-size:0.72rem; color:#CBD5E1; font-weight:600;">{_lbl_stat4_sub}</div>
        </div>
        """, unsafe_allow_html=True)
    ...
    with ann_head1:
        st.markdown(
            f'<div style="font-size:1.35rem; font-weight:900; color:#FFFFFF; letter-spacing:0.02em;">{_lbl_ann_section}</div>',
            unsafe_allow_html=True
        )
    with ann_head2:
        if st.button(_lbl_all_ann, key="btn_go_all_announcements", type="primary", use_container_width=True):
            st.query_params["view"] = "ann"
            st.rerun()
    ...
    if announcements:
        ...
        for i, a in enumerate(announcements[:4]):
            ...
            img_url = a.get("image_url") or fallback_covers[i % len(fallback_covers)]
            a_id = a.get("announcement_id", f"land_ann_{i}")
                ...
                <div style="background:#DEEFF4; border-radius:14px; overflow:hidden; display:flex; flex-direction:column; justify-content:space-between; min-height:360px; box-shadow:0 6px 20px rgba(0,0,0,0.25); border:1px solid rgba(61,211,255,0.3); transition:transform 0.2s ease;">
                    <div>
                        ...
                        <div style="background:#0E137A; padding:6px 12px; display:flex; justify-content:space-between; align-items:center;">
                            ...
                            <span style="background:#DE380F; color:#FFFFFF; font-size:0.68rem; font-weight:800; padding:2px 8px; border-radius:4px;">{_lbl_featured if is_pin else cat}</span>
                        </div>
    ...
    with c_center:
        is_auth = bool(st.session_state.get("authenticated", False))
        if is_auth:
            if st.button(_lbl_bottom_panel, key="tf_bottom_panel", type="primary", use_container_width=True):
                st.query_params.clear()
                st.query_params["tab"] = "ana_sayfa"
                st.rerun()
        else:
            if st.button(_lbl_bottom_login, key="tf_bottom_login", type="primary", use_container_width=True):
                st.query_params["view"] = "login"
                st.rerun()
