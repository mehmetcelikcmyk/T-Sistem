...
if "authenticated" not in st.session_state or not st.session_state.authenticated:
    ...
    if "code" in st.query_params or _view in ("login", "register") or st.session_state.get("auth_mode") == "google_complete_profile":
        if _view in ("login", "register"):
            ...
            with c_back:
                _back_lbl = "← Tanıtım ve Duyurulara Geri Dön" if st.session_state.get("lang", "tr") == "tr" else "← Back to Home & Announcements"
                if st.button(_back_lbl, key="btn_back_to_landing", type="secondary"):
...
if _invite_token:
    ...
    if not _invite_data:
        _invite_err = "Bu davet bağlantısı geçersiz veya süresi dolmuş. Lütfen kaptandan yeni davet isteyin." if st.session_state.get("lang", "tr") == "tr" else "This invitation link is invalid or has expired. Please request a new invite from the team captain."
        st.error(_invite_err)
    else:
...
st.markdown("""
<style>
/* TR / EN Pill Butonu — doğru key tabanlı selector */
div.st-key-lang-toggle-btn > button {
...
div.st-key-lang-toggle-btn > button:hover {
...
# Dil butonu — navbarın üzerinde, sağa yaslı
_lang_spacer, _lang_col = st.columns([5.5, 0.5])
with _lang_col:
    _other_lang_label = "🌐 EN" if current_lang == "tr" else "🌐 TR"
    _next_lang = "en" if current_lang == "tr" else "tr"
    if st.button(
        _other_lang_label,
        key="lang-toggle-btn",
        use_container_width=True,
        help="Switch language / Dili değiştir",
    ):
...
with n_col2:
    ...
    if rol in ("yarismaci", "uye"):
        ...
        with menu_c5:
            if st.button(t("nav_profile", current_lang), key="nav_profil", use_container_width=True, type="primary" if st.session_state.aktif_tab == "profil" else "secondary"):
                git_sekme("profil")
        with menu_c6:
            if st.button(t("nav_logout"), key="nav_logout_yarismaci", use_container_width=True):
    ...
    # 2. HAKEM / JÜRİ MENÜSÜ
        ...
        with menu_c4:
            if st.button(t("nav_profile", current_lang), key="nav_profil", use_container_width=True, type="primary" if st.session_state.aktif_tab == "profil" else "secondary"):
                git_sekme("profil")
        with menu_c5:
            if st.button(t("nav_logout"), key="nav_logout_hakem", use_container_width=True):
                auth_service.clear_active_session()
                st.session_state.authenticated = False
                st.session_state.user = None
                st.query_params.clear()
    ...
    # 3. YARIŞMA YÖNETİCİSİ MENÜSÜ
        ...
        with menu_c1:
            if st.button(t("nav_admin_yonetim", current_lang), key="nav_home", use_container_width=True, type="primary" if st.session_state.aktif_tab == "ana_sayfa" else "secondary"):
                git_sekme("ana_sayfa")
        with menu_c2:
            if st.button(t("nav_announcement_mgmt"), key="nav_announcements_top_mgr", use_container_width=True, type="primary" if st.session_state.aktif_tab == "yonetici_duyurular" else "secondary"):
        ...
        with menu_c4:
            if st.button(t("nav_profile", current_lang), key="nav_profil", use_container_width=True, type="primary" if st.session_state.aktif_tab == "profil" else "secondary"):
                git_sekme("profil")
        with menu_c5:
            if st.button(t("nav_logout"), key="nav_logout_yonetici", use_container_width=True):
                auth_service.clear_active_session()
                st.session_state.authenticated = False
                st.session_state.user = None
                st.query_params.clear()
    ...
    # 4. SİSTEM YÖNETİCİSİ / ADMİN MENÜSÜ
        ...
        with menu_c3:
            if st.button(t("nav_team_mgmt"), key="nav_admin_takimlar", use_container_width=True, type="primary" if st.session_state.aktif_tab == "admin_takimlar" else "secondary"):
        ...
        with menu_c5:
            if st.button(t("nav_profile", current_lang), key="nav_profil", use_container_width=True, type="primary" if st.session_state.aktif_tab == "profil" else "secondary"):
                git_sekme("profil")
        with menu_c6:
            if st.button(t("nav_logout"), key="nav_logout_admin", use_container_width=True):
                auth_service.clear_active_session()
                st.session_state.authenticated = False
                st.session_state.user = None
                st.query_params.clear()
